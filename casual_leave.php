...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($resultCheck === FALSE) {
        ...
        if ($resultCheck->num_rows > 0) {
            ...
            if ($resultLeaveBalance === FALSE) {
                ...
                if ($resultLeaveBalance->num_rows > 0) {
                    ...
                    $casualLeaveBalance = $leaveBalanceRow["Casual_Leave"];
                    ...
                    // Add debugging lines
                    echo "Casual Leave Balance: $casualLeaveBalance<br>";
                    echo "Requested Leave Duration: $dateDiff days<br>";

                    if ($casualLeaveBalance >= $dateDiff) {
        ...
        VALUES ('$employeeID', '$fromDate', '$toDate', '$day', '$reporting', 'casual_leave', '$approval')";


                        // Update leave balance after leave is granted
                        $updateLeaveBalanceQuery = "UPDATE user 
                                                    SET Casual_Leave = Casual_Leave - $dateDiff 
                                                    WHERE id = '$employeeID'";
        ...
        <div class="form-row">
            ...
            <select id="employeeID" name="employeeID" required>
                <?php foreach ($ids as $id): ?>
                    <option value="<?php echo $id; ?>" <?php echo ($employeeID == $id) ? "selected" : ""; ?>><?php echo $id; ?></option>
                <?php endforeach; ?>
            </select>
        </div>
