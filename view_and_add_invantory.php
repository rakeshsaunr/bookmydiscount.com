...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Split inventory_name by spaces
    $inventory_names = explode(' ', $inventory_name);

    // Insert each inventory name into Add_ivantory_name table
    $sql = "INSERT INTO Add_ivantory_name (store_id, store_name, inventory_id, inventory_name) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    
    foreach ($inventory_names as $name) {
        $stmt->bind_param("ssss", $store_id_manual, $store_name, $inventory_id, $name);
        if (!$stmt->execute()) {
